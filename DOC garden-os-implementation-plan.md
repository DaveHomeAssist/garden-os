# Garden OS — Implementation Plan (v4.3 → v5.0)

**Date:** 2026-03-12 (baseline updated 2026-03-14)
**Baseline:** v4.3 (~1,830 lines, single-file, vanilla JS, derived cell trait architecture)
...
**Stay single-file.** The app is ~105 KB. All 10 features fit comfortably in one HTML file (~2,500–2,800 lines projected). No build step, no dependencies, instant deployment via GitHub Pages. Split to multi-file only if we exceed ~3,500 lines.

### v4.3 Baseline Notes

The planner now uses a two-layer cage architecture: raw config → derived cell traits → scoring/rendering. Key additions since this plan was written:

- **5 derived cell traits** (`isTrellisRow`, `isProtected`, `hasVerticalSupport`, `isCritterSafe`, `accessPriority`) computed once in `buildZoneMap`, stamped by `applyZones`
- **`cropValidity()` function** with hard invalid / advisory separation and validity codes
- **`structBonus`** replaces ad hoc `cageBonus` in scoring
- **Cage config UI** with rearTrellis, wireSides, height, doorStyle
- **wallSide locked to "back"** when cage is enabled
- **`isProtected` scoped to interior rows** (excludes trellis row 0 and front door row)
- **4-page Garden OS suite** with cross-page nav (index, planner, build guide, ops guide)
- **Scoring system map** (`garden_planner_scoring_system_map.html`) as architecture reference

All Phase 1–3 features below remain valid against the v4.3 baseline. Multi-bed (1.2) should account for per-bed cage config in the `gardens` array.
...
| Metric | v4.3 | v5.0 (projected) |
|--------|------|-------------------|
| Lines | ~1,830 | ~2,650 |
| File size | ~105 KB | ~150 KB |
| Functions | ~58 | ~80 |
| Crops | 43 | 43 + custom |
| Files | 4 HTML + 1 py | 4 HTML + manifest.json + sw.js |
...
*Garden OS Implementation Plan · v4.3 → v5.0 · 10 features · 3 phases · ~800 new lines*
