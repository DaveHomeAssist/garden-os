...
| 3 | Y1 Fall | Adjacency scoring active — companion/conflict pairs affect yield | **Adjacency Preview** — green/red borders on hover showing companion/conflict | +carrot, +kale (cool-season) | Tomato Sandwich | — | GURL notices the bed looks intentional; Onion Man references the gardener's rows; Vegeman suggests buying instead of growing |
...
| 5 | Y2 Spring | Faction reputation tracking — GURL/Onion Man/Vegeman approval meters (hidden, behavior-driven) | **Faction Filter** — toggle to see which characters react to current layout | +eggplant, +chard | Pepper-Onion Skillet | — | GURL softens ("I see you fixed the setback"); Onion Man reveals he knows this bed's habits; Vegeman escalates (restaurant coupon) |
...
| 8 | Y2 Winter | Crop lineage — tracks which crops have been grown in which cells across seasons; soil memory | **Crop Lineage tags** — cell history popup showing past 4 seasons of that cell | No new crops (winter planning) | — | **The Photo** — the original bed photo, captioned for the active profile | Characters reference specific past decisions; Onion Man: "You rotated the tomatoes too" |
...
| Ch10 > Ch11 | Density scoring creates a soil pressure map — high-density cells need amendment or rotation next season | Full summer harvest. Herb variants contribute unique pantry items for sauce recipes. | "What this bed can hold" — density reflection, capacity acceptance | Peak bed complexity. Every cell has history, condition, lineage. | Mastery rank affects dialogue tone. High rank = peers. Low rank = continued mentorship. |
| Ch11 > Ch12 | Final fall harvest. Soil state becomes the "legacy baseline" — frozen as starting condition for free play. | **Mom's Sauce** ingredients consumed if recipe completed under the default label; custom profiles use the gardener's sauce label. Pantry state = legacy pantry. | "What this bed taught me" — final journal entry, written differently (first person, no game framing) | Bed condition = legacy condition. No more auto-repair. Player maintains from here. | Final character lines delivered. Free play uses shortened, familiar dialogue. |
...
| Ch10 > Ch11 | Dedicate 1 row to sauce recipe ingredients | Maintain current layout | — |
...
| 7 | Mom's Sauce | tomato (8), onion (3), pepper (2), basil (3), hot pepper (1), carrot (2), oregano (1) | Ch11 (oregano available Ch10, recipe card Ch11) | **Handwritten Sauce Card keepsake.** Default label uses Mom; custom profiles use the gardener's sauce label. Stain on the corner. | Apex recipe. Requires 7 different crops at volume. Pantry must be deep. Completing this is the emotional climax. |
...
### Hidden Household Recipes (Default: Mom Recipes)

Not in the recipe ladder. Not hinted in UI. Discovered only through journal clues — specific phrases in seasonal journal entries that reference meals and routines from the active gardener's household. With the default Mom profile, these appear as Mom recipes. Player must recognize the clue, then attempt the recipe from pantry ingredients.
...
| 2 | August Salad | Grow lettuce successfully in summer (hard — requires shade cloth or shade strategy) | "Lettuce all summer. Still not asking how." | lettuce (3), tomato (2), cucumber (1), basil (1) | Recipe card note: "Ice water. That's the trick." |
...
| 4 | Freezer Sauce | Complete Late-Summer Sauce Prep twice (two separate seasons) | "Always make extra. For January." | tomato (10), onion (2), basil (2), carrot (1) | Recipe card with freezer bag illustration and a date written in pen. |
...
| Hidden | The Returning Gardener's Bed | All 5 hidden household recipes completed. Layout reconstruction (current bed matches the original bed plan — hinted in The Photo keepsake). Bed density at "Serious Grower+" tier. Legacy Keeper rank achieved. | Ch12 (post-completion) |
...
| `layoutMatchScore` | % match to the original bed layout (revealed in The Photo) | Every season end |
...
| 4 | The Photo | Ch8 winter journal review — appears automatically in journal during Year 2 winter reflection | Ch8 | Faded photograph of the original bed, summer light. Bed layout partially visible (used for hidden rank layout matching). Default profile captions it as Mom's reference photo; custom profiles caption it as the household reference photo. | Permanent. The Photo is also a gameplay artifact — studying it reveals the original layout for the hidden rank. |
...
| 7 | Handwritten Sauce Card | Complete the apex sauce recipe (Ch11) | Ch11 | Index card, lined. Default profile labels it as Mom's card; custom profiles use the gardener's sauce card label. Tomato stain in upper right corner. No measurements — "enough tomatoes," "some basil," "you'll know." Flip side has a grocery list in different ink. | Permanent. Most significant keepsake. Displayed larger than others in the drawer. Only keepsake with a flip-side interaction. |
